...
whimsy.TestSuite(tests=tests, name='Truth Tests')
...
class CustomSuite(whimsy.TestSuite):
    pass
whimsy.config.defaultsuite = CustomSuite
...
class ReverseSuite(whimsy.TestSuite):
    def __iter__(self):
...
class TempfileFixture(whimsy.Fixture):
...
suite = whimsy.TestSuite(name='Tempfile Test Suite', fixtures=[TempfileFixture()])

class WriteTempTest(whimsy.TestCase):
    def test(self, test_parameters):
        tempfile_fixture = test_parameters.suite.fixtures[0]
        tempfile_fixture.file_.write('hello')
        tempfile_fixture.file_.flush()

class ReadTempTest(whimsy.TestCase):
    def test(self, test_parameters):
        tempfile_fixture = test_parameters.suite.fixtures[0]
...
class BuildSystemFixture(whimsy.Fixture):
    targets = []
    def setup(self, testitem):
        import subprocess
...
class SkipFixture(whimsy.Fixture):
    name = 'Test Skipper'
    def setup(self, testitem):
        testitem.skip()
...
class SkippedTest(whimsy.TestCase):
...
# For more advanced test writers whimsy enables modification of test running logic.
...
class IterativeRunner(whimsy.SuiteRunner):
    def handle_run(self):
        test_iter = iter(self.testitem)
        for test in test_iter:
            result = test.runner(test, self.suite).execute()
            if result == whimsy.State.Failed:
                whimsy.log.test_log.message(
        ...
        for test in test_iter:
            ...
            test.result = whimsy.State.Skipped
            whimsy.log.test_log.test_status(test, test.result)

class FailFastSuite(whimsy.TestSuite):
...
# # Configuration
# #################
# whimsy.config.defaultsuite
# whimsy.config.defaultsuiterunner
# whimsy.config.defaulttestrunner

# whimsy.config.defaultpathfilter

# # In order to collect tests items, whimsy executes the python files containing tests.
# # To prevent accidental running of unrelated scripts, by default whimsy will first check files for a top level import statement from whimsy.
# whimsy.config.checkforimport
...
# if not whimsy.collecting == __name__:
...
x86_suite = whimsy.TestSuite(tests=[Test(True, name='X86Test')], name='X86 Test Suite', tags=['X86'])
arm64_suite = whimsy.TestSuite(tests=[Test(True, name='ARM64Test')], name='ARM64 Test Suite', tags=['ARM64'])

# To do so, execute the run subcommand as follows::
# whimsy run --include-tags X86
#
#
# A note on tags: 
# Some might ask why tests do not have the tags attribute as well.
#
# The reason behind this goes back to whimsy's definition of TestCase and TestSuite.
...
#
# In order to keep a simpler and consistent interface, whimsy decides to only enable tags at the TestSuite level.
